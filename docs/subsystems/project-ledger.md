...
- The `/project` command reads: todo views by executor kind, the plan doctor, per-item reviews with observed evidence, the replay audit that compares the rebuilt projection with the materialized rows in both directions, and the evidence digest that aggregates plan versions, item completion with per-criterion verdicts, and the replay verdict — which `export` renders again as one archival markdown block.
