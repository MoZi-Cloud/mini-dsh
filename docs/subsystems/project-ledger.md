...
- The `/project` command reads: todo views by executor kind, the plan doctor, per-item reviews and evaluation histories with observed evidence, the replay audit that compares the rebuilt projection with the materialized rows in both directions, the evidence digest that aggregates plan versions, item completion with per-criterion verdicts, and the replay verdict — which `export` renders again as one archival markdown block — the owner decision requests with their options and resolving decisions (`/project decisions`), the approvals over their typed subjects (`/project approvals`), the resource requirements with their instances and verification verdicts (`/project resources`), and the registered actors, defined roles, and live assignments (`/project actors`).
...
Evaluations are append-only history and statuses are the projection; acceptance is the only completion authority, so no session todo or plan edit can shortcut work to done. Owner decisions and approvals are the same shape: a decision request opens once per project-scoped key and exactly one decision resolves it; an approval hangs beside a typed subject reference and decides exactly once; a resource requirement collects verified instances; actors, roles, and their live assignments give the actor strings a durable referent — all recorded as required events the fold replays. The doctor re-verifies an imported version in one pass — flagging even lease rows that still record ACTIVE past their own expiry, the rows a behind reaper owes recovery — the replay audit reports any divergence between events and rows, and neither mutates or executes a verifier command. Work packets carry only their recorded recipe; a rebuild recomposes the packet from current rows and names what drifted.
