...
Agents in the profile also get the three model-facing tools. `project_work_next` lists the calling agent's per-actor todo view — claim readiness, blockers, and lease holders, minus the items another agent holds a live claim on (v1.6d); `project_work_claim` takes the lease on one ready item and returns the bounded work packet — the objective, phase, blocking receipts, acceptance criteria, and stored verifier specs; `project_work_update` advances the held claim: `heartbeat` extends the lease, `release` gives the item back, and `report` records the agent's verifier observations — verdict plus optional exit code and bounded output tail, stored in each evaluation's observed payload — moving the item to `VERIFYING` or `FAILED` and completing it only when every required criterion already passes. The claim and heartbeat horizon is deployment config (`leaseTtlMs`, `leaseHeartbeatIntervalMs`).
