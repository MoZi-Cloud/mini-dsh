 ...
 * the event timeline is readable by this codec, and the replay audit reports
 * no drift between the replayed projection and the materialized tables.
 * Projection parity has one owner, `readProjectReplay`; the doctor converts
 * its drift into issues and adds only the runtime health check replay cannot
 * express (lease staleness). The doctor never
...
import { PROJECT_EVENT_FORMAT_VERSION } from './project-events.js'
import { readProjectReplay } from './project-replay.js'
...
export function planDoctor(
  ...
  const replay = readProjectReplay(db, projectId)
  if (replay.outcome === 'undecodable') {
    ...
    issues.push({
      ...
      message: `the project event timeline cannot be decoded by event format ${PROJECT_EVENT_FORMAT_VERSION}: ${replay.timelineError}`,
    })
  } else {
    for (const finding of replay.drift) {
      issues.push({ code: 'projection-drift', refId: finding.refId, message: finding.message })
    }
  }
