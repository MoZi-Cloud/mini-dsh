...
`dsh-experimental-project-ledger` owns the plan document seam of the v1.6a Ledger Core. `parsePlanDocument` parses YAML rejecting duplicate keys, anchors, aliases; `validatePlanSchema` mirrors the constitution schema; `validatePlanSemantics` checks references, relations. `compilePlan` builds canonical IR, `importPlanVersion` writes it atomically, `activatePlanVersion` activates through an owner event, the event seam replays fail-closed, leases arbitrate claimability, `buildWorkPacket` prepares bounded packets, todo views split work by executor, supersede with drift retire versions; replay, digest return per-item parity; v1.6b seams record decisions, approvals, verified resources, and actor roles beside their subjects; v1.6d records assignments, handoffs, scope reservations, and collaboration conflicts. Nothing here executes a verifier.
...
- **Atomic with events** — one `BEGIN IMMEDIATE` transaction writes the version rows and appends the `plan/imported` and `work/created` events; any failure rolls back to a ledger without partial rows. Import never activates: versions land as `DRAFT`.
...
- **Owner and Agent todos are executor-separated views** — `listOwnerTodo` and `listAgentTodo` run one read-only query over `executor_kind` (§11) across the non-terminal statuses, ordered by kind, descending priority, age, and id; every entry carries the recomputed readiness and the live lease, so blocking causes and holders surface with the task. A named viewer identity makes the view per-actor (v1.6d): entries another worker holds a live claim on drop out, so a second agent sees the queue without another agent's live claims. Views never mutate: completion stays with the acceptance seam, and a session todo — no stable item ids, no project identity — has no path into the ledger.
- **Activation is an owner action** — `activatePlanVersion` moves a `DRAFT` version to `ACTIVE` with its `activated_at_ms` stamp and the `plans.current_version_id` pointer inside one `BEGIN IMMEDIATE` with the `plan/version-activated` event; it refuses versions without a recorded import, versions that already left `DRAFT`, and plans that still hold an `ACTIVE` version (§8).
...
- **Actors and roles give the actor strings a durable referent** — `registerActor` records a project actor under `actor/registered` (`HUMAN`/`AGENT`/`SERVICE`/`SYSTEM`), `defineRole` records the `GOVERNANCE` or `EXECUTION` role an approval's `required_role` names under `role/defined`, and `assignRole` grants it under `role/assigned`, one live assignment per actor-role pair — the partial unique index and the seam both refuse a second. `INACTIVE` actors and ended actor-role assignments are reserved with no writer yet.
- **Work assignments record an actor's duty on an item** — `assignWorkItem` writes one of six uppercased duties (blueprint §18) beside the v1.6b actor, role, and item rows under `work/assigned`, with one live `PRIMARY` per item — the partial unique index, the seam, and the fold all refuse a second. `ENDED` and the acceptance and completion timestamps are reserved with no writer yet.
- **Handoffs record one pass of an item between actors** — `recordHandoff` writes the sender, exactly one recipient (an actor or a role), the `DELEGATE` or `RETURN` kind, and an inline summary under `handoff/recorded` (blueprint §28, its content indirection folded into summary text), with artifact and memory references serialized beside the row. `accepted_at_ms` is reserved with no writer yet.
- **Scope reservations keep one actor on one scope** — `reserveScope` writes an actor's exclusive claim on a project scope (a `PATH` value) beside the item it serves under `scope/reserved` (blueprint §21, its `mode` column dropped: every reservation ships exclusive), with one active reservation per project scope enforced by a partial unique index, the seam, and the fold; `releaseScopeReservation` frees the scope and `reapExpiredScopeReservations` moves the lapsed rows, `scope/released` and `scope/expired` each recording the transition. A stale row holding a scope expires inside the next reserve, exactly like the lease claim's reap.
- **Collaboration conflicts record overlaps the reservations slipped** — `recordConflict` writes one recorded overlap between two distinct work items of one project under `conflict/recorded`, raised by a registered actor with an inline description (blueprint §29, its content indirection inlined, its nullable referents closed to required, its `conflict_kind` closed to `SCOPE_OVERLAP` — the containment overlap the equal-value uniqueness check does not judge); `resolveConflict` closes it through a recorded v1.6b decision of the same project, `conflict/resolved` naming that decision.
- **Replay audits compare both directions** — `readProjectReplay` folds a project's event timeline and compares the rebuilt projection with the materialized tables, family by family — plan versions by identity facts, items, criteria, and leases by status, decisions and approvals by their resolving facts, resource requirements by identity facts, instances and verifications by their references, actors and roles by identity facts, actor roles by their pairs, work assignments by their duties, handoffs by their recipients, scope reservations by their scopes and lifecycle status, collaboration conflicts by their items, description, and resolution decision — so a row nothing replays and a replay nothing materializes are both drift. Work packets count on the replayed side only (the recipe is the durable record); a timeline this build cannot decode reports why instead of half-comparing. The audit only reads: parity is a fact, not a repair.
...
- **No supersede-edge adoption yet** — importing rejects work items that already belong to another version or the backlog, and incoming `SUPERSEDES` edges are excluded from readiness until the adoption flow lands.
...
- **Decision, approval, resource, actor/role, work-assignment, handoff, scope-reservation, and collaboration-conflict writes are library seams** — `openDecisionRequest`, `recordDecision`, `requestApproval`, `decideApproval`, `openResourceRequirement`, `provideResourceInstance`, `verifyResourceInstance`, `registerActor`, `defineRole`, `assignRole`, `assignWorkItem`, `recordHandoff`, `reserveScope`, `releaseScopeReservation`, `reapExpiredScopeReservations`, `recordConflict`, and `resolveConflict` have no slash command or model-facing tool yet (the `/project decisions`, `/project approvals`, `/project resources`, `/project actors`, `/project assignments`, `/project handoffs`, `/project reservations`, and `/project conflicts` views are read-only); interactive owner surfaces follow.
