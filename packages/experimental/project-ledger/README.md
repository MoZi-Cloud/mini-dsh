...
- **Decision writes are library seams** — `openDecisionRequest` and `recordDecision` have no slash command or model-facing tool yet (the `/project decisions` view is read-only), and approvals plus actor/role records are later v1.6b work; interactive owner surfaces follow the approvals item.
