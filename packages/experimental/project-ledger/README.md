...
- **Decision and approval writes are library seams** — `openDecisionRequest`, `recordDecision`, `requestApproval`, and `decideApproval` have no slash command or model-facing tool yet (the `/project decisions` and `/project approvals` views are read-only), and actor/role records are later v1.6b work; interactive owner surfaces follow.
