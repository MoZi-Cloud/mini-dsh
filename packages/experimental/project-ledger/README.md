...
`dsh-experimental-project-ledger` owns the plan document seam of the v1.6a Ledger Core. `parsePlanDocument` parses YAML rejecting duplicate keys, anchors, aliases; `validatePlanSchema` mirrors the constitution schema; `validatePlanSemantics` checks references, relations. `compilePlan` builds canonical IR, `importPlanVersion` writes it atomically, the event seam replays fail-closed, readiness and lease seams arbitrate claimability, `buildWorkPacket` prepares bounded packets, todo views split work by executor, supersede with drift retire versions and block moved baselines; review, history, replay, digest return per-item parity; v1.6b seams record decisions, approvals, verified resources, and actor roles beside their subjects; v1.6d records assignments, handoffs, scope reservations, and collaboration conflicts. Nothing here executes a verifier.
