...
- **Decision, approval, resource, actor/role, work-assignment, handoff, and scope-reservation writes are library seams** — `openDecisionRequest`, `recordDecision`, `requestApproval`, `decideApproval`, `openResourceRequirement`, `provideResourceInstance`, `verifyResourceInstance`, `registerActor`, `defineRole`, `assignRole`, `assignWorkItem`, `recordHandoff`, `reserveScope`, `releaseScopeReservation`, and `reapExpiredScopeReservations` have no slash command or model-facing tool yet (the `/project decisions`, `/project approvals`, `/project resources`, `/project actors`, `/project assignments`, `/project handoffs`, and `/project reservations` views are read-only); interactive owner surfaces follow.
