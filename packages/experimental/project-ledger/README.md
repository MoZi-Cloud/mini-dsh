...
`dsh-experimental-project-ledger` owns the plan document seam of the v1.6a Ledger Core. A plan document is inert: `parsePlanDocument` parses YAML, rejecting duplicate keys, anchors, and aliases; `validatePlanSchema` mirrors the constitution schema; `validatePlanSemantics` checks references and relations. `compilePlan` builds a canonical IR, `importPlanVersion` writes it atomically, the event seam replays fail-closed, `computeWorkReadiness` recomputes claimability, the lease seam arbitrates one lease per item, `buildWorkPacket` prepares bounded packets, todo views split work by executor kind, and supersede with drift retire versions and block moved baselines; `listPlans`, `readWorkItemReview`, `readProjectReplay`, and `readProjectDigest` read plans, reviews, parity, and the evidence digest. Nothing here executes a verifier command.
...
- **The digest aggregates, it never decides** — `readProjectDigest` reads every plan with each version's lifecycle status and baseline, every item's criteria tally with its latest verdict counts, and the embedded replay verdict, so an owner reads the whole record without SQL or per-item commands.
