...
`dsh-experimental-project-ledger` owns the plan document seam of the v1.6a Ledger Core. `parsePlanDocument` parses YAML rejecting duplicate keys, anchors, aliases; `validatePlanSchema` mirrors the constitution schema; `validatePlanSemantics` checks references, relations. `compilePlan` builds canonical IR, `importPlanVersion` writes it atomically, the event seam replays fail-closed, readiness and lease seams arbitrate claimability and one lease per item, `buildWorkPacket` prepares bounded packets, todo views split work by executor, and supersede with drift retire versions and block moved baselines; review, history, replay, and digest reads return per-item facts, parity, evidence; v1.6b seams record owner decisions, approvals, and verified resources beside their subjects. Nothing here executes a verifier.
...
- **Decision, approval, and resource writes are library seams** — `openDecisionRequest`, `recordDecision`, `requestApproval`, `decideApproval`, `openResourceRequirement`, `provideResourceInstance`, and `verifyResourceInstance` have no slash command or model-facing tool yet (the `/project decisions`, `/project approvals`, and `/project resources` views are read-only), and actor/role records are later v1.6b work; interactive owner surfaces follow.
