...
- **Decision, approval, resource, actor/role, and work-assignment writes are library seams** — `openDecisionRequest`, `recordDecision`, `requestApproval`, `decideApproval`, `openResourceRequirement`, `provideResourceInstance`, `verifyResourceInstance`, `registerActor`, `defineRole`, `assignRole`, and `assignWorkItem` have no slash command or model-facing tool yet (the `/project decisions`, `/project approvals`, `/project resources`, `/project actors`, and `/project assignments` views are read-only); interactive owner surfaces follow.
