...
- **协作冲突记录从预留漏掉的撞界**——`recordConflict` 在 `conflict/recorded` 之下写入一次已记录的撞界，挂在同一项目两个不同工作条目旁，由在册 actor 提出、附内联描述（蓝图 §29，其内容间接引用改为内联文本，可空引用收口为必填，`conflict_kind` 收口为 `SCOPE_OVERLAP`——等值唯一性检查判不到的包含型撞界）；`resolveConflict` 经同项目一条已记录的 v1.6b 决议收拢冲突，`conflict/resolved` 点名该决议。
- **重放对账双向比对**——`readProjectReplay` 折叠项目的全量事件时间线，把重建投影与物化表逐族比对——plan 版本按身份事实、工作项、标准与租约按状态、决策与审批按解除事实、资源需求按身份事实、实例与验证按引用、actor 与 role 按身份事实、actor-role 按其配对、工作指派按其职责、交接按其接收方、范围预留按其范围与生命周期状态、协作冲突按其条目、描述与解决决议——物化侧多出的行与重放侧多出的实体同样算 drift。WorkPacket 只计入重放侧（recipe 即持久记录）；本构建无法解码的时间线只报告原因，不做半程比对。对账只读：parity 是事实，不是修复。
